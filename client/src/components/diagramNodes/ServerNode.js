...
const ServerNode = ({ data, id }) => {
  ...
  const [tempLabel, setTempLabel] = useState(data?.label || 'Server');
  ...
  const handleLabelCancel = () => {
    setTempLabel(data?.label || 'Server');
    setIsEditingLabel(false);
  };
  ...
  return (
    <Card 
        ...
        sx={{ 
            ...
            textAlign: 'center',
            '&:hover': {
              boxShadow: '0 4px 8px rgba(85, 85, 85, 0.3)'
            }
        }}
    >
      <CardContent sx={{ padding: '8px !important' }}>
        <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
           <StorageIcon fontSize="medium" sx={{ mb: 0.5, color: '#555' }} />
