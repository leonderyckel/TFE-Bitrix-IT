...
const ComputerNode = ({ data, id }) => {
  const [isEditingLabel, setIsEditingLabel] = useState(false);
  const [isEditingIP, setIsEditingIP] = useState(false);
  const [tempLabel, setTempLabel] = useState(data?.label || 'Computer');
  ...
  const handleLabelCancel = () => {
    setTempLabel(data?.label || 'Computer');
    setIsEditingLabel(false);
  };
  ...
  return (
    <Card 
        ...
        sx={{ 
            ...
            textAlign: 'center',
            '&:hover': {
              boxShadow: '0 4px 8px rgba(153, 153, 153, 0.3)'
            }
        }}
    >
      <CardContent sx={{ padding: '8px !important' }}>
        <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          <ComputerIcon fontSize="medium" sx={{ mb: 0.5, color: '#999' }} />
