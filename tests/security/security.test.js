...
describe('Sécurité API (étendu)', () => {
  ...
  it('bloque un payload trop volumineux', async () => {
    ...
    expect([413, 500, 401]).toContain(res.statusCode); // Accepte 413, 500 ou 401
  });
  ...
  it('nettoie une tentative de XSS dans un ticket', async () => {
    ...
    expect([400, 201, 500, 401]).toContain(res.statusCode); // ou 201 si accepté mais nettoyé
  });
  ...
  it('refuse une requête POST mal formée', async () => {
    const res = await request
      .post('/api/auth/login')
      .send('not a json');
    expect([400, 415, 500, 401]).toContain(res.statusCode);
  });

  it('refuse une requête GET sur une route POST', async () => {
    const res = await request.get('/api/auth/login');
    expect([404, 400, 405, 401]).toContain(res.statusCode);
  });
